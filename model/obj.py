...
class OBJ:
    def __init__(self, filename, file=None, path=None):
        ...
        for line in open(filename, "r"):
            ...
            if values[0] == 'v':
                vals = map(float, values[1:4])
                vals[0] *= 0.075
                vals[1] *= 0.075
                vals[2] *= 0.075
                vertices.append(vals)
                self.minX = min(vals[0], self.minX)
                self.maxX = max(vals[0], self.maxX)
                self.minY = min(vals[1], self.minY)
                self.maxY = max(vals[1], self.maxY)
                self.minZ = min(vals[2], self.minZ)
                self.maxZ = max(vals[2], self.maxZ)
            elif values[0] == 'vn':
                normals.append(map(float, values[1:4]))
            elif values[0] == 'vt':
                tex_coords.append(map(float, values[1:3]))
            elif values[0] == 'mtllib':
                self.load_material_library(values[1]+'.mt')
